...
class Agent:
    def __init__(
        ...
        self.encoder = models.Encoder(
            obs_channels_or_dim=obs_shape[0] * history,
    ...
    def train(self):
        ...
        # compute targets
        Q_target, zs, zsa = self.compute_targets(
            state,
            action,
            next_state,
            reward,
    ...
    def train_encoder(
        ...
        with torch.no_grad():
            ...
            ns_flat = next_state.reshape(
                batch_size * self.enc_horizon, *self.state_shape
            )
        ...
        # compute loss
        encoder_loss = compute_encoder_loss(
            self.enc_horizon,
            pred_zs,
            target_zs,
            action,
    ...
    def get_next_action(
        self, action, target_policy_noise, noise_clip, next_zs, policy_target, discrete
    ):
        noise = (torch.randn_like(action) * target_policy_noise).clamp(
    ...
    def compute_targets(
        ...
        with torch.no_grad():
            ...
            Q_target = (
                reward + not_done * self.discount * Q_target_value * target_reward_scale
            ) / reward_scale
