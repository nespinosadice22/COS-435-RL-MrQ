...
class ReplayBuffer:
    def __init__(
        ...
        self.num_channels = obs_shape[
            0
        ]  # Used to grab only the most recent obs (history) or channels.
        ...
        self.ep_timesteps = 0
        self.env_terminates = (
            False  # Used to track if there are any terminal transitions in the buffer.
        )

        # History (used even if history = 1)
        self.history = history
        self.state_ind = np.zeros(
            (self.max_size, self.history), dtype=np.int32
        )  # Tracks the indices of the current state.
        self.next_ind = np.zeros(
            (self.max_size, self.history), dtype=np.int32
        )  # Tracks the indices of the next state.
        ...
        self.priority = (
            torch.empty(self.max_size, device=self.device) if self.prioritized else []
        )
        ...
        self.mask = torch.zeros(
            self.max_size, device=self.device if prioritized else torch.device("cpu")
        )
        ...
        self.action_reward_notdone = torch.zeros(
            (self.max_size, action_dim + 2), device=self.device, dtype=torch.float
        )
    ...
    def one_hot_or_normalize(self, action: int | float):
        ...
        return torch.tensor(
            action / self.action_scale, dtype=torch.float, device=self.device
        )
    ...
    def add(
        ...
        if (
            self.ep_timesteps > self.horizon
        ...
        self.state_ind[self.ind] = np.array(
            self.history_queue, dtype=np.int32
        )  # Track last x=history obs for the state.
        self.history_queue.append(
            next_ind
        )  # Update history queue with incremented ind.
    ...
    def terminal(self, state: np.array, truncated: bool):
        ...
        self.mask[(self.ind + self.history - 1) % self.max_size] = 0
        past_ind = (
            self.ind - np.arange(min(self.ep_timesteps, self.horizon)) - 1
        ) % self.max_size
        self.mask[past_ind] = (
    ...
    def sample(self, horizon: int, include_intermediate: bool = False):
        ...
        if include_intermediate:
            ...
            state_ind = np.concatenate(
                [self.state_ind[ind], self.next_ind[ind[:, -1].reshape(-1, 1)]], 1
            )
            both_state = (
                self.obs[state_ind]
                .reshape(self.batch_size, -1, *self.state_shape)
                .to(self.device)
                .type(torch.float)
            )
            state = both_state[:, :-1]  # State: (batch_size, horizon, *state_dim)
            next_state = both_state[
                :, 1:
            ]  # Next state: (batch_size, horizon, *state_dim)
        ...
        # Sample at specific horizon (used for multistep rewards).
            ...
            both_state = (
                self.obs[state_ind]
                .reshape(self.batch_size, 2, *self.state_shape)
                .to(self.device)
                .type(torch.float)
            )
    ...
    def load(self, save_folder: str):
        ...
        self.obs = torch.tensor(
            buffer_data["obs"], device=self.storage_device, dtype=self.obs_dtype
        )
        self.action_reward_notdone = torch.tensor(
            buffer_data["ard"], device=self.device, dtype=torch.float
        )
        self.state_ind = buffer_data["state_ind"]
        self.next_ind = buffer_data["next_ind"]
        if self.prioritized:
            self.priority = torch.tensor(buffer_data["priority"], device=self.device)
        self.mask = torch.tensor(
            buffer_data["mask"],
            device=self.device if self.prioritized else torch.device("cpu"),
        )
