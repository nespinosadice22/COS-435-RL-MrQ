...
class DefaultExperimentArguments:
    ...
    # just ensures all fields in a dataclass match
    # declared types by casting
    def __post_init__(self):
        utils.enforce_dataclass_type(self)


def main():
    # this is just so we can override defaults (-1 means default)
    ...
    # use GPT only if avail
    device = torch.device(
        "cuda" if torch.cuda.is_available() and args.device == "cuda" else "cpu"
    )

    # grabs defaults from above
