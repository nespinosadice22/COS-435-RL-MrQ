''' 
    Our main file is rewritten from scratch. We use Typer to take in arguments and Weights and Biases for logging. 
    Otherwise, we follow a pretty standard training/evaluation pattern. We add citations for places in which we referenced
    the original code (namely just in setting up hyperparameters). 
'''
...
# @citation: Hyperparameters used from https://github.com/facebookresearch/MRQ/tree/main
class Defaults:
    def __init__(self):
...
def main(
    ...
    #planning experiments only
    use_planning: bool = typer.Option(False, "--use-planning"), 
):
    #----------------------------------HYPERPARAMETER SET UP----------------------------------#
    # @citation: Parsing hyperparameter code taken from https://github.com/facebookresearch/MRQ/tree/main. 
    config = Defaults() 
    ...
    #-----------------------------------------SET UP WANDB-----------------------------------#
    run_config = { "env": env, "seed": seed,  "total_timesteps": total_timesteps, "device": str(device), 
        "eval_freq": eval_frequency, "eval_eps": eval_eps, "path": str(run_dir), "zs_dim": zs_dim, "za_dim": za_dim, 
        "zsa_dim": zsa_dim, "use_planning": use_planning, }
    run = wandb.init(name= run_name, project=project_name, group=f"{env}-{kind}", mode="offline",
        entity="ak5005-princeton-university", config=run_config, dir=str(run_dir))
    #----------------------------------DEBUGGING/TRACKING PRINT LOGIC-------------------------#
    eval_file = run_dir / "evals.txt"
    flush_eval = eval_file.open("a")
    print(f"RUN DETAILS: env = {str(env)} | seed = {seed} | total_timesteps = {total_timesteps}  | kind = {kind} | device = {device}", file=flush_eval, flush=True)
    if kind == "embeddings": 
        print(f"         | zs = {zs_dim} | zsa = {zsa_dim} | za = {za_dim}", file=flush_eval, flush=True)
    elif use_planning:
        print(f"PLANNING") 
    else: 
        print(f"REP ONLY BASELINE") 
    print(f"{'STEP'}  {'MEAN_R':>12s}", file=flush_eval, flush=True) 
    #---------------------------------ENV PREPROCESSING-----------------------------------------#
    ...
    def evaluate(step: int): 
        ...
        print(f"STEP: {step:>10d}, REWARD: {mean_reward:>10.3f} TIME (min) {elapsed_time}", file=flush_eval, flush=True)
    #------------------------------------------TRAIN!---------------------------------------------#
    for t in range(t_start, total_timesteps + 1): 
        ...
        if terminated or truncated: 
            wandb.log({"train/step": t + 1, "train/episode": env.ep_num, "train/episode_length": env.ep_timesteps, "train/episode_reward": env.ep_total_reward})
